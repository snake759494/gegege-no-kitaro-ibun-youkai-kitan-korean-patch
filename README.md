...
일본판 **ゲゲゲの鬼太郎 異聞妖怪奇譚 (SLPM-65337, 코나미, 2003)**용 비공식 한국어 패치 **v1.0.0**입니다. 키타로와 요괴들이 등장하는 PS2 시뮬레이션 RPG의 게임 내 텍스트 전부, 대사 글꼴, 캐릭터 이름표, 전투·메뉴 UI 스프라이트와 메뉴 배경을 한글화했습니다.
...
릴리즈 첨부 파일은 **`Kitaro_Ibun_Youkai_Kitan_PS2_KO_v1.0.0.xdelta` 하나**입니다. 저장소에는 제작 소스(`tools/`), 번역 원본(`script_ko.tsv`), 대조용 원문 덤프(`script_jp.tsv`), 글리프 대응표와 검증 기록을 공개합니다. 원본 및 패치 적용 ISO, 게임에서 추출한 바이너리, 글꼴 TTF, 스크린샷, 외부 실행 파일은 배포하지 않습니다. GitHub의 Source code ZIP/TAR는 공개 소스 압축본입니다.
...
.\xdelta3.exe -d -s '.\Gegege no Kitarou Ibun Youkai Kitan.iso' '.\Kitaro_Ibun_Youkai_Kitan_PS2_KO_v1.0.0.xdelta' '.\Kitaro_KR.iso'
...
```powershell
python apply_release.py --xdelta '.\xdelta3.exe' --source '.\Gegege no Kitarou Ibun Youkai Kitan.iso' --patch '.\Kitaro_Ibun_Youkai_Kitan_PS2_KO_v1.0.0.xdelta' --output '.\Kitaro_KR.iso'
```
...
- **UI 이미지** — `D_SYS.BIN` 스프라이트 99장(51개 컨테이너): 반격 명령·작전 목적·저장·불러오기·설정·막간 화면, 돌판 태그 5장(조작 가능/적극 반격/효율 반격/반격 불가/항상 방어), 상태 이상 7종, 명·회·공·방·출·완 아이콘, 결착, 로딩 중, 챕터 타이틀 44장, 휴대폰 요괴 링크 안내문·버튼, 상점 정렬 버튼·안내문, 모노노케WEB·요괴 "y" 쇼핑 로고, 세로쓰기 장소 간판. `D_BG.BIN` 메뉴 배경 9장: 요괴 링크, 요괴 "y" 쇼핑(안내·구입·매각), 모노노케WEB, 요괴타임스 기사·토픽스, 요괴 아파트 간판(결정/뒤로, L1·R1 페이지 라벨, 소지금·소유수·금액·원 라벨 포함).
- **의도적으로 남긴 것** — 타이틀 화면의 게임 로고(`D_BG.BIN` #66), 요괴타임스의 눈알 일러스트 로고, KONAMI 로고는 원본 아트 그대로입니다.
- **미적용** — `D_EFF.BIN`의 아이템 효과 라벨 11장(体力回復 등, 컨테이너 형식이 달라 미해석), RenderWare 텍스처(`D_TXD.BIN`, `D_BTTF~1.BIN`의 맵 재질·간판, PS2 스위즐 형식). 이 부분은 게임 중 일본어로 남습니다.
...
- **자동 검수** — `tools/qa_text.py`(왕복 검증, 커버리지, 잔류 가나·한자, 줄 수·줄 폭, 제어 코드, 용어 일관성, 서브파일 용량 7항목)와 `tools/qa_img.py`(크기, 컨테이너 헤더·레코드·팔레트 무손상, 변경 바이트가 의도한 스프라이트 안쪽인지, 대비 4항목)를 통과했습니다. 결과 전문은 [qa_text_report.txt](qa_text_report.txt), [qa_img_report.txt](qa_img_report.txt)입니다.
...
- **xdelta 왕복** — 원본 ISO에 xdelta를 적용한 결과가 빌드한 ISO와 바이트 단위로 같음을 확인했습니다(SHA-1 `396bbb0c…` 일치).
- **재현성** — 저장소의 소스와 번역 원본으로 다시 만든 패치 파일 5개가 배포본과 바이트 단위로 같은지 [release_verification.json](release_verification.json)에 기록했습니다.
- **에뮬레이터** — PCSX2 2.2.0에서 한글 글꼴 표시(인트로 내레이션, 빈 글리프 블록 접근)와 이름표는 이전 빌드에서 확인했습니다. 최종 v1.0.0 빌드의 인게임 확인 범위는 [실행 확인 기록](runtime_test.md)에 구분해 두었습니다. 전체 챕터 완주와 실제 PS2 기기는 미검증입니다.
