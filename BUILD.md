...
python tools/patch_bg.py                         # (참고용) extract/D_BG.BIN -> D_BG_ko.BIN. v1.0.1부터 배포에 넣지 않음 — 넣으면 프롤로그에서 멈춤
python tools/qa_text.py                          # 텍스트 자동 검수 (RESULT 줄이 no blocking issues 여야 함)
python tools/qa_img.py                           # 이미지 자동 검수
python tools/build_iso.py "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR.iso D_MOJI.BIN=D_MOJI_hangul2350.BIN D_MESS.BIN=D_MESS_ko.BIN D_CHFONT.BIN=D_CHFONT_ko.BIN D_SYS.BIN=D_SYS_ko.BIN
...
이 저장소의 소스로 다시 만든 패치 파일 4개가 v1.0.0 배포본과 바이트 단위로 같은지는 [release_verification.json](release_verification.json)의 `rebuild` 항목에 기록했습니다. 빌드는 에뮬레이터를 실행하지 않습니다. 공개 저장소만으로 게임 이미지를 만들 수는 없습니다.
