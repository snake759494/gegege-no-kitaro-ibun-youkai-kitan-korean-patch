...
python tools/patch_sys.py                        # extract/D_SYS.BIN -> D_SYS_ko.BIN
python tools/patch_bg.py                         # extract/D_BG.BIN -> D_BG_ko.BIN
python tools/qa_text.py                          # 텍스트 자동 검수 (RESULT 줄이 no blocking issues 여야 함)
python tools/qa_img.py                           # 이미지 자동 검수
python tools/build_iso.py "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR.iso D_MOJI.BIN=D_MOJI_hangul2350.BIN D_MESS.BIN=D_MESS_ko.BIN D_CHFONT.BIN=D_CHFONT_ko.BIN D_SYS.BIN=D_SYS_ko.BIN D_BG.BIN=D_BG_ko.BIN
.\xdelta.exe -e -9 -f -s "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR.iso Kitaro_KR.xdelta
.\xdelta.exe -d -f -s "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR.xdelta roundtrip.iso   # 해시가 Kitaro_KR.iso와 같아야 함
```

모든 스크립트는 저장소 루트를 기준 경로로 잡습니다(`ko_nameplates.py`, `ko_inject.py`는 루트에서 실행). `patch_bg.py`는 `extract/D_BG.BIN`이 없으면 루트의 원본 ISO에서 직접 읽습니다. 생성 파일 5개는 모두 원본과 같은 크기이며 `build_iso.py`는 크기가 다르면 중단하고, 각 파일을 ISO 안의 원래 LBA 위치(D_MOJI 1899, D_MESS 2623, D_SYS 3021, D_BG 56654, D_CHFONT 85908)에 덮어쓴 뒤 다시 읽어 확인합니다. 파일 시스템은 재구성하지 않습니다.
...
이 저장소의 소스로 다시 만든 패치 파일 5개가 v1.0.0 배포본과 바이트 단위로 같은지는 [release_verification.json](release_verification.json)의 `rebuild` 항목에 기록했습니다. 빌드는 에뮬레이터를 실행하지 않습니다. 공개 저장소만으로 게임 이미지를 만들 수는 없습니다.
