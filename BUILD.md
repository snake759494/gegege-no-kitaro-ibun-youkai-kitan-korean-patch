...
New-Item -ItemType Directory -Force validation_20260916
...
python tools/qa_img.py --sys validation_20260916/D_SYS_candidate.BIN --bg validation_20260916/D_BG_candidate.BIN
python tools/test_image_safety.py
python tools/build_iso.py "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR_v1.0.3.iso D_MOJI.BIN=D_MOJI_hangul2350.BIN D_MESS.BIN=D_MESS_ko.BIN D_CHFONT.BIN=D_CHFONT_ko.BIN D_SYS.BIN=validation_20260916/D_SYS_candidate.BIN D_BG.BIN=validation_20260916/D_BG_candidate.BIN
.\xdelta.exe -e -9 -f -s "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_KR_v1.0.3.iso Kitaro_Ibun_Youkai_Kitan_PS2_KO_v1.0.3.xdelta
.\xdelta.exe -d -f -s "Gegege no Kitarou Ibun Youkai Kitan.iso" Kitaro_Ibun_Youkai_Kitan_PS2_KO_v1.0.3.xdelta roundtrip.iso   # 해시가 Kitaro_KR_v1.0.3.iso와 같아야 함
```

v1.0.3는 대사·글꼴·이름표와 SYS·BG 이미지를 패치합니다. 위 명령은 공개 v1.0.3 구성입니다. 이미지 재적용 검증 규칙과 미검증 실행 범위는 [IMAGE_VALIDATION.md](IMAGE_VALIDATION.md)를 참고하세요. **출력 ISO는 반드시 존재하지 않는 새 경로를 지정하세요.** 수정한 `build_iso.py`는 기존 출력을 거절하여 이전 이미지 패치가 남는 것을 방지합니다.
...
기존 대사·글꼴·이름표 3개 파일의 소스 재빌드 일치 기록과 이번 SYS·BG 검증 결과는 [release_verification.json](release_verification.json)에 기록했습니다. 빌드는 에뮬레이터를 실행하지 않습니다. 공개 저장소만으로 게임 이미지를 만들 수는 없습니다.
